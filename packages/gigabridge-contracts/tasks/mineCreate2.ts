...
//
// Only the total number of fixed characters decides the cost — 16× per character, wherever it sits,
// since one guess either matches everywhere or it does not. Three zeros plus a four-character suffix
// is the same work as a seven-character prefix: seconds at 5, minutes at 7, hours at 9. Past ~8 this
// stops being the right tool — hand `create2MiningTarget(artifact)` to create2crunch or
...
export default task("mine-create2", "Mine CREATE2 salts for leading-zero addresses with a chosen ending")
    ...
    .setInlineAction(async ({ contracts, zeros, suffix, threads, outDir }, hre) => {
        ...
        // Addresses are compared lowercased, so the case anyone types the suffix in is irrelevant —
        // it is not a checksum, just 4 bits per character.
        const tail = suffix.toLowerCase().replace(/^0x/, "");
        if (!/^[0-9a-f]*$/.test(tail)) {
        ...
        const label = `0x${"0".repeat(wanted)}…${tail}`;
        ...
        // between fixing 7 characters and 9 is minutes and hours.
        console.log(`expect ~${(16 ** (wanted + tail.length)).toExponential(1)} attempts per contract`);
        ...
        for (const contract of contracts as string[]) {
            ...
            const salt = await mine(initCodeHash, pattern, workerCount, contract);
            ...
            // will compute rather than whatever the worker's byte fiddling produced.
 ...
 *
 * `pattern` is what the address has to look like, in the form {@link matchesPattern} describes.
 */
async function mine(
